...
Matd MultiLinearHardeningPlasticSolid::PlasticConstitutiveRelation(const Matd &F, size_t index_i, Real dt)
{
    ...
    Matd deviatoric_PK = DeviatoricKirchhoff(normalized_be - normalized_be_isentropic * Matd::Identity());
    Real deviatoric_PK_norm = deviatoric_PK.norm();
    Real sigma = hardening_stresses_(hardening_parameter_[index_i])(0);                   // get the yield stress corresponding to the current hardening parameter from the spline
    hardening_modulus_[index_i] = hardening_moduluses_(hardening_parameter_[index_i])(0); // get the hardening modulus corresponding to the current hardening parameter from the spline

    Real trial_function =
        deviatoric_PK_norm - (sqrt_2_over_3_ * sigma); // the trial function which is part of the strain calculation is based on the current stress

    // check if the trial function is greater than zero at every yield point past the intial yield stress
    if (trial_function > 0.0)
    {
        Real renormalized_shear_modulus = normalized_be_isentropic * G0_;
