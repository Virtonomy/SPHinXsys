...
{
public:
    ObersverAxial(SPHBody &sph_body, double full_length, Vec3d translation = Vec3d(0.0,0.0,0.0)) : ObserverParticleGenerator(sph_body)
    {
        int ny = 51;
        for (int i = 0; i < ny; i++)
        {
...
{
public:
    ObserverRadial(SPHBody &sph_body, double full_length, double diameter, int number_of_particles, Vec3d translation = Vec3d(0.0,0.0,0.0)) : ObserverParticleGenerator(sph_body)
    {
...
const Vec3d emitter_buffer_halfsize = Vec3d(fluid_radius, inflow_length*0.5 ,fluid_radius);
...
const BoundingBox system_domain_bounds(Vec3d(-diameter , 0, -diameter) - Vec3d(wall_thickness, wall_thickness, wall_thickness), Vec3d(diameter , full_length, diameter) + Vec3d(wall_thickness, wall_thickness, wall_thickness));
/**
...
{
public:
	explicit ShellBoundary(SPHBody &sph_body) : SurfaceParticleGenerator(sph_body){};
	void initializeGeometricVariables() override
	{
			for (int i = 0; i < particle_number_mid_surface + 2 * BWD; i++)
			{
				...
				{
					Real x = radius_mid_surface * cos( Pi + (i - BWD + 0.5) * 2 * Pi / (Real)particle_number_mid_surface);
					...
					initializePositionAndVolumetricMeasure(Vec3d(x, y, z), resolution_ref * resolution_ref);					
...
int main()
{
	...
	SimpleDynamics<TimeStepInitialization> initialize_a_fluid_step(water_block, makeShared<Gravity>(Vec3d(0.0, 0.0, 0.0)));
	/**
	...
	Dynamics1Level<fluid_dynamics::FluidShellIntegration2ndHalfRiemann> density_relaxation(water_block_complex);
	...
	SimpleDynamics<fluid_dynamics::EmitterInflowInjection> emitter_inflow_injection(emitter, 10, 1);
	/** Emitter buffer inflow condition. */
	BodyAlignedBoxByCell emitter_buffer(
		water_block, makeShared<AlignedBoxShape>(Transform3d(Vec3d(emitter_buffer_translation)), emitter_buffer_halfsize));
	...
	BodyAlignedBoxByCell disposer(
		water_block, makeShared<AlignedBoxShape>(Transform3d(Vec3d(disposer_translation)), disposer_halfsize));
	SimpleDynamics<fluid_dynamics::DisposerOutflowDeletion> disposer_outflow_deletion(disposer, 1);
    ...
    observer_radial.generateParticles<ObserverRadial>(full_length, diameter, number_of_particles);
    ...
    ObservedQuantityRecording<Vec3d> write_fluid_velocity_radial("Velocity", io_environment, observer_contact_radial);
	/**
