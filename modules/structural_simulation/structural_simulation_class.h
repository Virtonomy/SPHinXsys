...
{
public:
	ImportedModel(SPHSystem &system, string body_name, TriangleMeshShape* triangle_mesh_shape, ParticleAdaptation* particle_adaptation, StdLargeVec<Vecd>& pos_0, StdLargeVec<Real>& volume);
	~ImportedModel();
};
...
public:
	SolidBodyForSimulation(
		SPHSystem &system, string body_name, TriangleMeshShape& triangle_mesh_shape,ParticleAdaptation& particle_adaptation,
		Real physical_viscosity, LinearElasticSolid& material_model, StdLargeVec<Vecd>& pos_0, StdLargeVec<Real>& volume);
	~SolidBodyForSimulation(){};
