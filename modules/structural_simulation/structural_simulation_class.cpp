...
{	
	...
	BoundingBox bounds = mesh->findBounds();
...
ImportedModel::ImportedModel(
	...
	ParticleAdaptation* particle_adaptation, StdLargeVec<Vecd>& pos_0, StdLargeVec<Real>& volume)
	: SolidBody(system, body_name, particle_adaptation, new ParticleGeneratorDirect())
{
	// the particles are generated by the direct generator
	std::unique_ptr<ComplexShapeTriangleMesh> mesh(new ComplexShapeTriangleMesh());
	ComplexShape original_body_shape(mesh.get());
	mesh->addTriangleMeshShape(triangle_mesh_shape, ShapeBooleanOps::add);

	body_shape_ = new LevelSetComplexShape(this, original_body_shape, true, false);
	...
	{
		body_input_points_volumes_.push_back(std::make_pair(pos_0[particle_i], volume[particle_i]));
	}
}
...
SolidBodyForSimulation::SolidBodyForSimulation(
	...
	imported_model_(ImportedModel(system, body_name, &triangle_mesh_shape, &particle_adaptation, pos_0, volume)),
...
std::tuple<StdLargeVec<Vecd>, StdLargeVec<Real>> getParticlesFromTriangleMeshShape(TriangleMeshShape* triangle_mesh_shape, Real resolution, bool particle_relaxation, bool write_particle_relaxation_data)
{
	BoundingBox bb = triangle_mesh_shape->findBounds();
	SPHSystem system(bb, resolution);
	
	ParticleAdaptation particle_adaptation(1.15, 1.0);
	...
	LevelSetComplexShape body_shape(&model, original_body_shape, true, false);
	model.body_shape_ = &body_shape;
...
void StructuralSimulation::initializeAllContacts()
{
	...
	{	
		...
		SolidBodyRelationContact* contact = new SolidBodyRelationContact(contact_body, target_list);
...
{	
	...
	for (size_t i = 0; i < surface_pressure_tuple_.size(); i++)
	{
		...
		TriangleMeshShape* tmesh =get<1>(surface_pressure_tuple_[i]);
		Vec3d point = get<2>(surface_pressure_tuple_[i]);
		StdVec<array<Real, 2>> pressure_over_time = get<3>(surface_pressure_tuple_[i]);
		
		BodyPartByParticle* bp = new BodyPartByParticleTriMesh(solid_body_list_[body_index]->getImportedModel(), imported_stl_list_[body_index], tmesh);
...
{	
	...
	for (size_t i = 0; i < surface_spring_tuple_.size(); i++)
	{
		...
		TriangleMeshShape* tmesh = get<1>(surface_spring_tuple_[i]);
...
void StructuralSimulation::runSimulation(Real end_time)
{
	...
	if(surface_particles_only_to_vtu_) write_states_surface.writeToFile(0);
	else write_states.writeToFile(0);
