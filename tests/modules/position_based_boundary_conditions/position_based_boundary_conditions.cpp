...
TEST(StructuralSimulation, PositionSolidBodyTuple)
{
	...
	//=================================================================================================//
	// test the shape, if the bounding box is correct
	int large_limit = 1e3; // this is already 1km of distance, no coordinate should be larger

	TriangleMeshShape ball_mesh("./input/ball_mass.stl", translation_list[0] * scale_stl, scale_stl);
	BoundingBox bounds = ball_mesh.findBounds();
	std::cout << "bounds.first: " << bounds.first << std::endl;
	std::cout << "bounds.second: " << bounds.second << std::endl;
	ASSERT_LT(std::abs(bounds.first[0]), large_limit);
	ASSERT_LT(std::abs(bounds.first[1]), large_limit);
	ASSERT_LT(std::abs(bounds.first[2]), large_limit);
	ASSERT_LT(std::abs(bounds.second[0]), large_limit);
	ASSERT_LT(std::abs(bounds.second[1]), large_limit);
	ASSERT_LT(std::abs(bounds.second[2]), large_limit);


	bounds = sim.get_position_solid_body_()[0]->getSPHBody()->getBodyDomainBounds();
	std::cout << "bounds.first: " << bounds.first << std::endl;
	std::cout << "bounds.second: " << bounds.second << std::endl;
	ASSERT_LT(std::abs(bounds.first[0]), large_limit);
	ASSERT_LT(std::abs(bounds.first[1]), large_limit);
...
StdLargeVec<Vecd>& pos_n = sim.get_solid_body_list_()[0].get()->getElasticSolidParticles()->pos_n_;
	...
	// TODO: improve accuracy and decrease tolerance to 1e-6
}
...
TEST(StructuralSimulation, TranslateSolidBodyPartTuple)
{
	...
	for (size_t index = 0; index < pos_0.size(); index++)
	{
		...
		{
			Vec3d end_pos = pos_0[index] + translation_vector;
			EXPECT_NEAR(pos_n[index][2], end_pos[2], end_pos.norm() * 0.02); // 2% tolerance
		}
		else
		{
