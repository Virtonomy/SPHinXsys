...
public:
    SimTotalArtificialHeart(SimTotalArtificialHeartInput & input)
    {
        ...
        std::vector<Vec3d> translation_list = {
            Vec3d(input.translation_tah[0], input.translation_tah[1], input.translation_tah[2]),
            Vec3d(0),
            Vec3d(0),
            Vec3d(0),
            Vec3d(0),
        ...
        std::cout << "LinearElasticSolid" << std::endl;
        LinearElasticSolid material_tah = LinearElasticSolid(input.rho_0, input.Youngs_modulus_tah, input.poisson);
        std::cout << "NeoHookeanSolid" << std::endl;
        NeoHookeanSolid material_vessel = NeoHookeanSolid(input.rho_0, input.Youngs_modulus, input.poisson);
        std::vector<LinearElasticSolid> material_model_list = { material_tah,    material_vessel, material_vessel,
                                                                material_vessel, material_vessel, material_vessel };

        /** INPUT DECLERATION */
        StructuralSimulationInput inputStructuralSim { input.relative_input_path, imported_stl_list,
                                                       input.scale_stl,           translation_list,
                                                       resolution_list,           material_model_list,
                                                       input.physical_viscosity,  input.contacting_bodies_list };
        inputStructuralSim.non_zero_gravity_ = std::vector<GravityPair> {
            GravityPair(0, Vec3d(0.0, 45.0, 0.0))
        }; // gravity for TAH
        ...
        /** SIMULATION MODEL */
        std::cout << "new StructuralSimulation()" << std::endl;
        sim.reset(new StructuralSimulation(&inputStructuralSim));
        std::cout << "SimTotalArtificialHeart end" << std::endl;
    };

    ~SimTotalArtificialHeart() {};

public: // C++ Backend functions
    void runCompleteSimulation(double endTime)
    {
        sim->RunSimulation(SPH::Real(endTime));
    };

public: // WASM functions
    void runSimulationFixedDurationJS(int number_of_steps)
    {
        sim->RunSimulationFixedDurationJS(number_of_steps);
    };
