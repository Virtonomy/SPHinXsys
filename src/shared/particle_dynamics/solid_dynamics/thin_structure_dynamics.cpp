        ...
        { /// Enforce plane stress condition adapting algorithm from Sec. 5.4.1 from http://dx.doi.org/10.18419/opus-14215
          ...
          ///       even more so considering we do not reuse analytical derivatives.
            ...
            if (cauchy_stress.allFinite() == false || it == max_iterations)
                throw std::runtime_error("[ShellStressRelaxationFirstHalf::initialization] Enforcing plane stress condition failed for particle with unsorted_id: " + std::to_string(unsorted_id_[index_i]) + " in object: " + sph_body_.getName() + " with normal stress in the out-of-plane direction: " + std::to_string(cauchy_stress(2, 2)););
        }
