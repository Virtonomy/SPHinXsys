...
void BarParticles::initializeOtherVariables()
{
    ...
    for (size_t i = 0; i != real_particles_bound_; ++i)
    {
        ...
        numerical_damping_scaling_[i](Dimensions - 2, Dimensions - 2) =
            thickness_[i] < sph_body_.sph_adaptation_->ReferenceSmoothingLength() ? width_[i] : sph_body_.sph_adaptation_->ReferenceSmoothingLength();
    }
}
} // namespace SPH
